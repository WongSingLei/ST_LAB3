...
import static ut.math.BigDecimal.INFLATED;
import static ut.math.BigInteger.LONG_MASK;

import ut.math.BigDecimal;
import ut.math.BigInteger;
import ut.math.SignedMutableBigInteger;
...
class MutableBigInteger {
     ...
     */
    MutableBigInteger(ut.math.BigInteger b) {
        intLen = b.mag.length;
     ...
     */
    ut.math.BigInteger toBigInteger(int sign) {
        if (intLen == 0 || sign == 0)
            return ut.math.BigInteger.ZERO;
        return new ut.math.BigInteger(getMagnitudeArray(), sign);
    }

    /**
     * Converts this number to a nonnegative {@code BigInteger}.
     */
    ut.math.BigInteger toBigInteger() {
        normalize();
     ...
     * and scale.
     */
    ut.math.BigDecimal toBigDecimal(int sign, int scale) {
        if (intLen == 0 || sign == 0)
            return ut.math.BigDecimal.zeroValueOf(scale);
        ...
        if (len > 2 || (d < 0 && len == 2))
            return new ut.math.BigDecimal(new ut.math.BigInteger(mag, sign), INFLATED, scale, 0);
        long v = (len == 2) ?
    ...
    public String toString() {
        ut.math.BigInteger b = toBigInteger(1);
        return b.toString();
    }
    ...
    void rightShift(int n) {
        ...
        if (nBits == 0)
            return;
        int bitsInHighWord = ut.math.BigInteger.bitLengthForInt(value[offset]);
     ...
     */
    private ut.math.BigInteger getLower(int n) {
        if (isZero()) {
            return ut.math.BigInteger.ZERO;
        } else if (intLen < n) {
            ...
            while (len > 0 && value[offset+intLen-len] == 0)
                len--;
            int sign = len > 0 ? 1 : 0;
            return new ut.math.BigInteger(Arrays.copyOfRange(value, offset+intLen-len, offset+intLen), sign);
        }
    }
    ...
    MutableBigInteger divide(MutableBigInteger b, MutableBigInteger quotient, boolean needRemainder) {
        if (b.intLen < ut.math.BigInteger.BURNIKEL_ZIEGLER_THRESHOLD ||
                intLen - b.intLen < ut.math.BigInteger.BURNIKEL_ZIEGLER_OFFSET) {
    ...
    MutableBigInteger divideAndRemainderBurnikelZiegler(MutableBigInteger b, MutableBigInteger quotient) {
        ...
        if (r < s) {
            ...
            int m = 1 << (32-Integer.numberOfLeadingZeros(s/ ut.math.BigInteger.BURNIKEL_ZIEGLER_THRESHOLD));
    ...
    private MutableBigInteger divide2n1n(MutableBigInteger b, MutableBigInteger quotient) {
        ...
        // step 1: base case
        if (n%2 != 0 || n < ut.math.BigInteger.BURNIKEL_ZIEGLER_THRESHOLD) {
    ...
    private MutableBigInteger divide3n2n(MutableBigInteger b, MutableBigInteger quotient) {
        ...
        ut.math.BigInteger b2 = b.getLower(n);
    ...
    private MutableBigInteger modInverse(MutableBigInteger mod) {
        ...
        ut.math.SignedMutableBigInteger c = new ut.math.SignedMutableBigInteger(1);
        ut.math.SignedMutableBigInteger d = new ut.math.SignedMutableBigInteger();
        MutableBigInteger temp = null;
        ut.math.SignedMutableBigInteger sTemp = null;
